...
#Mortality rate for ages 1-101 i.e. q1,q2,...,q101 from ONS mortality rate
...
PMF[102-age]=prod(neg)
#Inputs the last value in PMF to represent P[x>101], as we don't have mortality rates past this age
...
axis(1,at=c(0:(102-age)),cex.axis=0.4)

barplot(PMF,space=0,main='PMF',ylab='p',xlab='years left') #plots PMF
axis(1,at=c(0:(102-age)),cex.axis=0.5)
...
ExEx=sum(c(seq(age+1,101),102)*PMF)-age-0.5
ExEx
#This calculates expected life expectancy, i.e. the mean years left to live
#-0.5 to adjust for the fact that person doesn't die in exactly n years but lies in (n-1,n]
