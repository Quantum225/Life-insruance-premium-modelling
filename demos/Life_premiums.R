...
barplot(DDay,space=0,axes=FALSE,ylim=c(0,90))
...
#MortR=c(q1,q2,q3,q4,q5,...q102)
#dec=c(1-q1,1-q1,...,1-q102)

DeathD=numeric(102)

for (age in 0:101) {
  
  Placeholder=numeric(102-age)
  
  for (n in 1:(102-age)) {
    Placeholder[n]=MortR[age+n]*(age+n)*prod(if (n<=1) NULL else neg[(age+1):(age+n-1)])
  }
  DeathD[age+1]=sum(Placeholder)-0.5-age
}

DeathD
par(mfrow = c(1,1))
barplot(DeathD,space=0,axes=FALSE,ylim=c(0,90),main='deterministic')
axis(1,at=seq(0,100,10),cex.axis=0.9)
axis(2,at=seq(0,90,5),cex.axis=0.8)

barplot(DDay,space=0,axes=FALSE,ylim=c(0,90),main='simulation 10000')
axis(1,at=seq(0,100,10),cex.axis=0.9)
axis(2,at=seq(0,90,5),cex.axis=0.8)
...
age=10
for (n in 1:(102-age)) {
  PMF[n]=MortR[age+n] * prod(if (n<=1) NULL else neg[(age+1):(age+n-1)]);
}
barplot(PMF)
par(mfrow=c(1,2))
...
for (n in 1:nsim) {
  ...
  years_left[n]=which.max(Death*dec)-1
  death_age[n]=which.max(Death*dec)+age-1
}

bins=cut(years_left, breaks=seq(0,100,1),right=FALSE)
...
years_left
barplot(bin_count)
quantile(years_left,0.95)


#```````````````````````````````````````````````rate of imrpovement

roi=rep(0.995,102)
roi=runif(102,0.98,1)
#The rate of improvement(RoI) for the mortality year of each age, position 1 in vector meaning
#the RoI of q1, and etc.
delay=1
#Was this data from 1 year ago, 2 years ago? If 1 year ago set delay=1

p2=roi[(age+1):length(roi)]
p22
for (a in 1:(length(roi)-age)) {
  p22[a]=p2[a]^(delay+a-1)
}

p1=MortR[(age+1):length(MortR)]
p=p1*p22
Death=rbinom(length(p),1,p)
dec=seq(from=length(p),to=1,by=-1)
Death*dec
